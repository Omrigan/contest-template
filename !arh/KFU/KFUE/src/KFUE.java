...
public class KFUE {
    public static String filename = "KFUE".toLowerCase();
...
class Task {
    ...
    public void run() {
        ...
        int m = in.nextInt();
        for (int i =0;i<m;i++){
            int startAt = in.nextInt()-1, endAt = in.nextInt()-1;
            String change = in.next();
            String changer = in.next();
            while(s.indexOf(change.charAt(0),startAt)<endAt){
                s.charAt(s.indexOf(change.charAt(0))) = changer.charAt
            }
        }
    }
