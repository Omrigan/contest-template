...
public class Kr1F {
    public static String filename = "Kr1F".toLowerCase();
...
class Task {
    ...
    public void run() {
        int k = in.nextInt(), l;
        char[] str = (in.next() + "1").toCharArray();
        l = str.length;
        int left = 0, right = 1;
        int curk = 0;
        int ans = 0;
        ...
        if (curk == k)
            while (right < l) {
                ...
                right++;
                while (right < l && str[right - 1] != '1') {
                    ansr++;
                    right++;

                }
                left++;
                while (left < right && str[left - 1] != '1') {
